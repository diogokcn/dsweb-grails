...
class AutorLivroController {
    ...
    def save(AutorLivro autorLivro) {
        ...
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.created.message', args: [message(code: 'autorLivro.label', default: 'AutorLivro'), autorLivro.id])
                redirect autorLivro
            }
            '*' { respond autorLivro, [status: CREATED] }
        }
    }
